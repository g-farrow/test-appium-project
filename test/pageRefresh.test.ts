...
// @ts-ignore
import { setupBrowser } from '@testing-library/webdriverio' // eslint-disable-line
...
const getAuthToken = async (
  ...
  const response = await fetch(process.env.GENERATE_PASSCODE_URL!, {
    ...
    body: JSON.stringify({
      ...
      redeemableFrom: Math.floor(
        redeemableFrom.setMinutes(redeemableFrom.getMinutes() + 0) / 1000
      ),
      redeemableTo: Math.floor(
        redeemableTo.setMinutes(redeemableTo.getMinutes() + 10) / 1000
      ),
      numberOfLicenses: 1
    })
  })
  const { passcode } = await (response.json() as Promise<{
    passcode: string
  }>)
